...
## 🧠 ДОКТРИНА УПРАВЛЕНИЯ ФАЙЛАМИ (ОБНОВЛЕНО 01.08.2025)

### **Основные принципы:**

1. **📄 Парсинг в TXT формат** - все файлы (PDF, DOCX, EPUB, FB2, DOC) парсятся в TXT и сохраняются в файловом хранилище
2. **💾 Файловое хранилище** - конвертированные TXT файлы сохраняются в файловом хранилище (возможно в `public/` для доступности скачивания)
3. **🔐 Дедупликация по хешу** - проверка дубликатов по SHA-256 хешу файла перед обработкой
4. **✅ Условное сохранение** - файл сохраняется в хранилище ТОЛЬКО если парсинг И эмбеддинг прошли успешно
5. **🗑️ Очистка при ошибках** - если парсинг не удался, файл стирается и данные о нём тоже удаляются
6. **📊 Мета-данные в БД** - при успешной обработке хеш и мета-информация сохраняются в основной базе для дедупликации

### **Flow обработки файлов (НОВАЯ ДОКТРИНА):**

```
📁 Upload → 🔐 Hash Check → ❓ Exists? → 🔄 Parse to TXT → 🔗 Embed → ✅ Success? → 💾 Save to Storage → 📊 Store Meta in DB
                                                                        ❌ Fail? → 🗑️ Delete File & Data
```
...
5. **Создание эмбеддингов** - добавление в векторную базу (Qdrant)
6. **Проверка успешности** - если парсинг И эмбеддинг прошли успешно:
   - ✅ **Сохранение в файловое хранилище** (возможно `public/storage/`)
   - ✅ **Запись мета-данных в БД** (хеш, размер, тип, путь к файлу)
   - ✅ **Файл доступен для скачивания пользователю**
7. **Обработка ошибок** - если что-то пошло не так:
   - ❌ **Удаление файла** из временного хранилища
   - ❌ **Удаление всех данных** о файле из системы
