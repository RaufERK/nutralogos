...
export async function POST(request: NextRequest) {
  try {
    console.log('📤 [CLEAN UPLOAD] Processing file upload request')
    ...
    console.log(
      `📁 [CLEAN UPLOAD] File: ${file.name} (${file.size} bytes, ${file.type})`
    )
    ...
    console.log(
      `🔧 [CLEAN UPLOAD] Using processor: ${processor.constructor.name}`
    )
    ...
    console.log(`📝 [CLEAN UPLOAD] Extracted ${text.length} characters`)
